...
@interface BrowsingHistoryViewController()<UITableViewDataSource, UITableViewDelegate, CLLRefreshHeadControllerDelegate>
{
    NSInteger loadCount;
    UIButton *_btn;
}
...
- (void) addUI
{
    ...
    for (NSInteger i = 1; i < 3; i++) {
        ...
        {
            if (i == 1) {
                [btn setTitle:@"浏览历史" forState:UIControlStateNormal];
                btn.selected = YES;
                btn.userInteractionEnabled = NO;
            }else if(i == 2)
            {
                [btn setTitle:@"话题历史" forState:UIControlStateNormal];
                _btn = btn;
